;(function() {	
	...
	function saveCSVFile(){
		...
		chrome.storage.sync.get( null, function(r) {
			...
			let header = '\ufeff' + '"Présence pour:","' + ccn + '"\n"Date:","' + cdate + '",\n "Time:", "' + sessionStorage.getItem( 'Meeting-start-time' ) + '",\n "Meet ID:", "' + sessionStorage.getItem( '_activeMeetIDs' ) + '"\n'
			let cno=sessionStorage.getItem( 'class-notes' )||''
			header = header + ( cno == '' ? '' : 'Class notes:, "' + cno.trim() + '"') + '\n'
			header = header + '"Names", "\tEmail", "\tCommentaires", "\tHeure d Arrivée", "\tDernière Vue", "\t# of Checks", "\tJoined", "Details"\n'

;(function() {	
	...
	function saveCSVFile(){
		...
		chrome.storage.sync.get( null, function(r) {
			...
			let header = '\ufeff' + '"Présence pour:","' + ccn + '"\n"Date Cours:", "' + cdate + '", "Heure:", "' + sessionStorage.getItem( 'Meeting-start-time' ) + '", "Meet ID:", "' + sessionStorage.getItem( '_activeMeetIDs' ) + '"\n'
			let cno=sessionStorage.getItem( 'class-notes' )||''
			header = header + ( cno == '' ? '' : 'Class notes:, "' + cno.trim() + '"') + '\n'
			header = header + '"Noms Etudiant", "Email", "Commentaire", "Heure d arrivée", "Dernière Vue", "# of Checks", "Rejoindre", "Details"\n'
		...
		waitForElement( '[data-call-ended="true"]',function(){
			...
			chrome.storage.sync.get( null, function(r) {
				...
				if(!gmaEnabled){
					...
					asf="N'oubie pas de sauvegarder ce fichier!"
					asfh=30000
